...
export class DottedGridPrimitive implements ISeriesPrimitive<Time> {
  ...
  attached(param: SeriesAttachedParameter<Time>): void {
    ...
    param.chart.timeScale().subscribeVisibleLogicalRangeChange(() => {
      this._gridPaneView.update({
        xCoords: this.timeMarks.map((mark) => mark.coord),
        yCoords: this.priceMarks.map((mark) => mark.coord),
      })
    })
  }
  ...
  get timeMarks(): { coord: number }[] {
    console.log('cartcrom', this.pane._private__timeScale)

    return Object.values(this.pane._private__timeScale._private__timeMarksCache)
  }

  get priceMarks(): { coord: number }[] {
    return Object.values(this.pane._private__rightPriceScale._private__marksCache)
  }
}
